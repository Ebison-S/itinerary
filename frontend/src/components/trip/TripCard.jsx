...
export default function TripCard({ trip, selectable = false, selected = false, onToggleSelect }) {
  ...
  const media = (
    <div className="relative h-36 overflow-hidden">
      ...
      {selectable ? (
        <div
          className={clsx(
            'absolute top-3 left-3 w-7 h-7 rounded-full flex items-center justify-center border-2 transition-all',
            selected ? 'bg-coral border-coral' : 'bg-cream-paper/90 border-cream-paper backdrop-blur'
          )}
        >
          {selected && <Check className="w-4 h-4 text-cream-paper" strokeWidth={3} />}
        </div>
      ) : (
  ...
  if (selectable) {
    return (
      <button
        ...
        className={clsx(
          'tile block overflow-hidden text-left w-full transition-all duration-200',
          selected ? 'ring-2 ring-coral' : 'hover:-translate-y-1'
        )}
      >
        {media}
        {body}
  ...
  return (
    <Link to={`/trips/${trip.id}`} className="tile tile-hover block overflow-hidden group">
      {media}
      {body}
    </Link>
  );
